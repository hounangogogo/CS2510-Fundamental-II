 ...
 * 
 */
public class Frog implements FrogConstant {
    ...
    /**
     * To set the location of this frog to the given location produces the new
     * location
     * 
     * @param CartPt
     *            new location
    ...
    public CartPt moveBy(int dx, int dy) {
        ...
        if ((this.loc.x + dx >= 0) && (this.loc.x + dx <= width)
                && (this.loc.y + dy <= height)) {
    ...
    public boolean outOfScreen() {
        return !(this.floater == null)
                && ((this.loc.x + this.floater.velocityX <= 0) || (this.loc.x
                        + this.floater.velocityX >= width));
    }
}
